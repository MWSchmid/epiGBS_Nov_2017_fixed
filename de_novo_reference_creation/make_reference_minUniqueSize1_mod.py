...
def dereplicate_reads(in_files,args):
    ...
    for strand in ['watson','crick']:
        for name in ["demethylated_joined","merged_demethylated"]:
            ...
            cmd = [vsearch +' --minuniquesize 1 --derep_fulllength %s --sizeout --output %s'%(file_in,file_out)]
...
def make_uc(in_files,args):
    ...
    cmd = [vsearch + ' --minuniquesize 1 --derep_fulllength %s -strand both -uc %s'%(fasta_input,uc_out.name)]
...
def make_ref_from_uc(in_files,args):
    ...
    for line in uc_handle:
        if line.startswith('H') or line.startswith('S'): # commented by marc to really take all of the sequences
...
def main():
    ...
    files = {'combined':{"uc_out":"/media/localData/tempCRSAFR/tmp/combined_out5vRQ3a.uc"}} 
